...
if (root) {
  void (async () => {
    try {
      ...
      const payment = detail.payment as Record<string, string | number> | null;
      ...
      const paymentLabels: Record<string, string> = { pending: 'Pago pendiente', processing: 'Pago en proceso', paid: 'Pago confirmado', failed: 'Pago fallido', expired: 'Pago vencido' };
      content.innerHTML = `<div class="page-heading"><div><span class="badge">${escapeHtml(order.commercial_status)}</span><h1>${escapeHtml(order.order_number)}</h1><p>Creado ${new Intl.DateTimeFormat('es-MX', { dateStyle: 'long', timeStyle: 'short' }).format(new Date(String(order.placed_at)))}</p>${payment ? `<p><strong>${escapeHtml(paymentLabels[String(payment.status)] || String(payment.status))}</strong></p>` : ''}</div><strong class="price">${formatMoney(Number(order.total_amount_minor), String(order.currency_code))}</strong></div>
